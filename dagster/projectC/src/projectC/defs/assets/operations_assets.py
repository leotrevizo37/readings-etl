...
@graph_multi_asset(
    outs={
        "dwh_factReadingsMeasurement": AssetOut(
            key=AssetKey(["projectC", "dwh_factReadingsMeasurement"])),
        "ctl_IngestionControl" : AssetOut(
            key=AssetKey(["projectC", "ctl_IngestionControl"])),
    },
    ins={
        "data_transformations": AssetIn(
            key=AssetKey(["projectC", "data_transformations"]), dagster_type=Nothing),
    },
    group_name="projectC",
)
