...
def _day_numbers_from_schedule(column) -> F.Column:
    normalized = F.regexp_replace(_normalize_days(column), r"\s+", "")
    parts = F.split(normalized, ",")
    expanded_days = F.array_sort(
        F.array_distinct(
            F.flatten(
                F.transform(
                    parts,
                    lambda part: F.when(
                        F.instr(part, "-") > 0,
                        F.sequence(
                            F.element_at(F.split(part, "-"), 1).cast("int"),
                            F.element_at(F.split(part, "-"), 2).cast("int"),
                        ),
                    ).otherwise(F.array(part.cast("int"))),
                )
            )
        )
    )
    return F.when(
        normalized.isNull() | (normalized == F.lit("")),
        F.expr("cast(array() as array<int>)"),
    ).otherwise(expanded_days)


def _expand_schedule_days(df: DataFrame, days_column: str) -> DataFrame:
    return (
        df.withColumn(days_column, _normalize_days(F.col(days_column)))
        .withColumn("day_numbers", _day_numbers_from_schedule(F.col(days_column)))
        .withColumn("day_count", F.size(F.col("day_numbers")))
        .filter(F.col("day_count") > 0)
        .withColumn("day_number", F.explode("day_numbers"))
    )


def _prefer_more_specific_schedule(
    df: DataFrame,
    partition_columns: list[str],
    time_column: str,
    days_column: str,
) -> DataFrame:
    choice_window = Window.partitionBy(*partition_columns, time_column).orderBy(
        F.col("day_count").asc(),
        F.col(days_column).asc(),
    )
    return (
        df.withColumn("_schedule_rank", F.row_number().over(choice_window))
        .filter(F.col("_schedule_rank") == 1)
        .drop("_schedule_rank")
    )


def _day_numbers_to_string(day_numbers_column) -> F.Column:
    day_text = F.array_join(
        F.transform(day_numbers_column, lambda day: day.cast("string")),
        ",",
    )
    return F.when(day_text == F.lit("1,2,3,4,5,6,7"), F.lit("1-7")).otherwise(day_text)


def _compact_schedule_days(df: DataFrame, grouping_columns: list[str]) -> DataFrame:
    return (
        df.groupBy(*grouping_columns)
        .agg(F.array_sort(F.collect_set("day_number")).alias("day_numbers"))
        .withColumn("scheduled_days", _day_numbers_to_string(F.col("day_numbers")))
        .drop("day_numbers")
        .select(*grouping_columns, "scheduled_days")
    )
...
def build_hvac_programs(
    ...
    return (
        ...
        .select(
            ...
            F.col("pf.Mode"),
            (F.col("pf.Temperature")).alias(
                "Temperature"
            ),
            (F.col("pf.TemperatureGrace")).alias(
                "temperature_grace"
            ),
            F.col("ps.Days"),
...
def build_hvac_expected_values(
    ...
    hvac_dimensions = active_dimensions.filter(_is_hvac_device_type("device_type"))

    temperature_dimensions = hvac_dimensions.filter(
        F.col("sensor_type").isin(*HVAC_TEMPERATURE_SENSOR_TYPE)
    ).select(
    ...
    programs_prepared = (
        hvac_programs_df
        .withColumn("scheduled_days", _normalize_days(F.col("Days")))
    ...
    programs_by_day = _prefer_more_specific_schedule(
        _expand_schedule_days(programs_prepared, "scheduled_days"),
        ["device_id", "day_number"],
        "time_value",
        "scheduled_days",
    )

    if programs_by_day.rdd.isEmpty():
        return _empty_internal_expected_values_df(spark)

    ordered_window = Window.partitionBy("device_id", "day_number").orderBy("time_ts")
    grouped_window = Window.partitionBy("device_id", "day_number")
...
def build_lights_expected_values(
    ...
    lights_parsed = (
        ...
        .withColumn(
            "minute_of_day",
            F.col("cron_hour_int") * F.lit(60) + F.col("cron_minute_int"),
        )
    )
    ...
    lights_by_day = _prefer_more_specific_schedule(
        _expand_schedule_days(lights_parsed, "days_candidate"),
        ["sensor_id", "day_number"],
        "start_time_candidate",
        "days_candidate",
    )

    if lights_by_day.rdd.isEmpty():
        return _empty_internal_expected_values_df(spark)

    ordered_window = Window.partitionBy("sensor_id", "day_number").orderBy("minute_of_day")
